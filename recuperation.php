...
            if($valid){
                $verification_mail = $DB->query("SELECT nameUser, emailUser, mdpUser 
                    FROM user WHERE emailUser = ?",
                    array($mail));
                $verification_mail = $verification_mail->fetch();
 
                if(isset($verification_mail['emailUser'])){
                    if($verification_mail['mdpUser'] == 0){
...
                        $to = $verification_mail['emailUser'];
...
                        $DB->insert("UPDATE user SET mdpUser = ?, n_mdp = 1 WHERE emailUser = ?", 
                            array($new_pass_crypt, $verification_mail['emailUser']));
                    }   
                }       
